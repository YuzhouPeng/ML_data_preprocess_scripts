...
xml对应图片绝对路径   文件夹名称  含物体个数   第一个物体对应bbox对应4个坐标       第一个物体对应bbox对应物体名称
0001.jpg             filename       1           Xmin Ymin Xmax Ymax    破洞
'''
class Xml_Txt_IOU():
    ...
    def xml_txt_IOU(self):
        ...
        root = sys.path[0]
        print(root)
        txt_file = open(root+'\\large_pics_remarked123.txt','w')

        # 存有jpg及xml的文件夹
        files_part = 'C:\\Users\\admin\\Desktop\\large_pics\\'
        # files_part = os.path.join(file, "original_remarked")


        xml_files = glob.glob(os.path.join(files_part, '*/*.xml'))
        xml_files.sort()

        # 遍历所有xml文件，将文件写入txt中
        for xml_dir in xml_files:
            # str.replace('a','b') 将字符串中的a替换为b
            file_name = xml_dir.split('\\')[-2]
