...
classes = ["wear_safetybelt"]
outputpath = '/home/pengyuzhou/workspace/output_txt_path/'
...
def voc2yolo(path):
    ...
    for file in os.listdir(path):
        if file[-3:]=='txt':
            ...
            out_file = open(outputpath + file.replace('txt', 'txt'), 'w')
            # print(label_file)
            ...
            for i in range(1,len(line)):
                ...
                width = eles[3]
                height = eles[4]
                out_file.write(str(9) + " " + " ".join(str(x) for x in eles[1:5]) + '\n')
            ...
            #         out_file.write(str(9) + " " + " ".join(str(x) for x in bbox) + '\n')
            print("end generate file ")
if __name__ == '__main__':
	# 这里要改成自己数据集路径的格式
    path = '/home/pengyuzhou/workspace/CVC05-PartOcclusion/Annotations/txt/'
    voc2yolo(path)