    ...
    if flag == True:
        ...
        for parent, _, files in os.walk(file_path):
            for file in (files):
                ...
                jpgpath = resultpath+'/'+str(rwidth)+"X"+str(rheight)+'/'+file+"/"
                ...
                for w in range(widthcount + 1):
                    for h in range(heicount + 1):
                        if w != widthcount and h != heicount:
                            ...
                            cropped.save(jpgpath + str(h) + '_' + str(w) + '.jpg')
                print("结束剪切")
                result.append([img,croppedimg,file,rwidth,rheight,widthcount,heicount])
        return result
...
def merge_pics(origin_large_img, output_path, cut_size, flag, coords, class_names,filename,rwidth,rheight,widthcount,heicount):
    # 根据坐标画出ROI图像
    def draw_outputs(img, coord, row, col,rwidth,rheight,widthcount,heicount):
        ...
        if col != widthcount and row != heicount:
            ...
            x0y0 = tuple((rwidth - cut_size + x0, y0 + row * cut_size))
            x1y1 = tuple((rwidth - cut_size + x1, y1 + row * cut_size))
            img = cv2.rectangle(img,
                                x0y0, x1y1,
                                (255, 0, 0), 2)
            img = cv2.putText(img, '{} {:.4f}'.format(
                coord[4], coord[5]),
                              (rwidth - cut_size + x1, y1 + row * cut_size), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,
                              (0, 0, 255), 2)
        elif row == heicount and col != widthcount:
            x0y0 = tuple((x0 + col * cut_size, rheight - cut_size + y0))
            x1y1 = tuple((x1 + col * cut_size, rheight - cut_size + y1))
            img = cv2.rectangle(img,
                                x0y0, x1y1,
                                (255, 0, 0), 2)
            img = cv2.putText(img, '{} {:.4f}'.format(
                coord[4], coord[5]),
                              (x1 + col * cut_size, rheight - cut_size + y1), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,
                              (0, 0, 255), 2)
        elif row == heicount and col == widthcount:
            x0y0 = tuple((rwidth - cut_size + x0, rheight - cut_size + y0))
            x1y1 = tuple((rwidth - cut_size + x1, rheight - cut_size + y1))
            img = cv2.rectangle(img,
                                x0y0, x1y1,
                                (255, 0, 0), 2)
            img = cv2.putText(img, '{} {:.4f}'.format(
    ...
    if flag:
        ...
        for coord in (coords):
            ...
            for img_c in img_coord:
                print(img_c)
                draw_outputs(img, img_c, row, col,rwidth,rheight,widthcount,heicount)
        # print(resultlist)
        print("---------end---------")
        output_final_path = output_path+'/'+str(rwidth)+"X"+str(rheight)+'/' + filename+'/'
        if not os.path.exists(output_final_path):
            os.makedirs(output_final_path)
        cv2.imwrite(output_final_path+filename, img)


# ----------------------------------------------------


def detect_faults(cut_size,r_width, r_height):
    # 全局变量
    #剪切后原图及剪切后图像数组
    image_crop_results = []
    # crop函数读取大图路径
    ...
    for parent, _, files in os.walk(file_path):
        print("-------------开始剪切程序------------")
        image_crop_results = crop_image(file_path, cut_size, True, r_width, r_height, resultpath)
        print("------------结束剪切程序--------")

        # 读取crop文件夹中的文件，获取文件和检测框的信息
        #origin_pic为原图，croppedimglist为二维数组，每个元素含有剪切后图像，row和col
        for origin_pic,croppedimglist,filename,rwidth,rheight,widthcount,heicount in image_crop_results:
            ...
            #调用绘制检测框函数
            merge_pics(origin_large_img, output_path, cut_size, True, coords, class_names,filename,rwidth,rheight,widthcount,heicount)

        # 将剪切图片检测出的ROI绘制到大图上，保存至output_path
    t2 = time.time()
    print("total time: {}".format(t2-t1))
if __name__ == '__main__':
    detect_faults(416,2560,1920)
    detect_faults(416,1280,960)
    detect_faults(416,640,480)
